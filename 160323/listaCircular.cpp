...
void mostrarLista(LUE &lista){
    if (lista.size == 0) cout << "Lista vazia\n" << endl;
    else {
        cout << "Mostrando lista: \n" << endl;
        int cont = 0;
        Node *aux = lista.valores[cont];
        while (cont < lista.size){
            cout << "index " << cont << ": " << aux->info << endl;
            aux = aux->elo;
        }
        cout << endl;
        cout << "INDEX INICIAL: " << lista.start << endl;
        cout << "INDEX final?: " << lista.end << endl;
...
bool inserir(LUE &lista, char valor){
    ...
    // Comentado pois não lembro se inserir condicional era planejado ou não
...
bool inserirFinal(LUE &lista, char valor){
    ...
    if (lista.size == 0){ // Lista vazia
        novoNode->elo = NULL;
        lista.valores[lista.end++] = novoNode;
        return true;
    } else {
        if (lista.size == TAM-1){
            novoNode->elo = lista.valores[lista.start]; // Circular
        } else {
            novoNode->elo = NULL;
        }
        
        lista.valores[lista.end++]->elo = novoNode;
        return true;
    }
}
